...
*/
	...
	if (mq_getattr(mq, &attrib) == -1) {
		...
		printf("Message queue flags: %ld\n"
		       ...
		       attrib.mq_flags,
		       attrib.mq_maxmsg, attrib.mq_msgsize, attrib.mq_curmsgs);
	}
	...
	if ((pid = fork()) == -1) {
		...
		if (pid) {	// pai
			...
			do {
				...
				if ((n_bytes =
				     mq_receive(mq, msg, attrib.mq_msgsize, &prio)) == -1) {
					perror("Erro em mq_receive");
				} else {
					msg[n_bytes] = '\0';
					printf
					    ("Mensagem recebida: (size: %d) (prio: %u) %s\n",
					     n_bytes, prio, msg);
					fflush(stdout);
				}
			} while ((strncmp(msg, "fim", 3)));
